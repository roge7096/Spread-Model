...
Create a list of games, summarize EPA data by game and by team for both offense and defense, then join the EPA data to the list of games (there will be four splits: home-offense, visitor-offense, home-defense, visitor-defense). This will be used to feed the linear regression.
```{r}
...
Create plots to analyze the data prior to running the regression
```{r}
...
Run the linear regression and the regression tree
```{r}
# Linear Regression
...
Obtain a list of teams with their most recent roll_EPA_per_play, this will be used to predict spreads.
```{r}
...
Join the offensive and defensive model to the season schedule, modifying the data as necessary, and adding in predictions.
```{r}
...
Join the market data to the model data and create fields for betting suggestions
```{r}
